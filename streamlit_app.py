...
if uploaded_file:
    ...
    if run_button:
        ...
        def validate_kseb_petition(pdf_path: str) -> tuple:
            ...
            Checks for KSEB-specific keywords in first 30 pages.
            """
            try:
                import pdfplumber
                required_keywords  = ['kseb', 'truing up', 'sbu']
                supporting_keywords = ['kerala', 'generation', 'arr', 'kserc',
                                       'depreciation', 'interest', 'tariff']
                found_required   = set()
                found_supporting = set()
                page_count = 0

                with pdfplumber.open(pdf_path) as pdf:
                    page_count = len(pdf.pages)
                    pages_to_check = min(30, page_count)
                ...
                if missing:
                    return False, (
                        ...
                        f"Please upload the correct KSEB petition PDF."
                    ), {'pages': page_count, 'found': list(found_required)}

                # At least 3 supporting keywords for confidence
                if len(found_supporting) < 3:
                    return False, (
                        f"PDF does not contain sufficient KSERC regulatory content. "
                        f"Please ensure you are uploading the full KSEB truing-up petition."
                    ), {'pages': page_count, 'found': list(found_required)}

                return True, "Valid KSEB petition detected.", {
                    'pages': page_count,
                    'keywords_found': list(found_required) + list(found_supporting)
                }
